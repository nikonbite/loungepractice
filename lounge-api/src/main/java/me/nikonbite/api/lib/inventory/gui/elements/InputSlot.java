...
import me.nikonbite.api.lib.inventory.gui.events.GUIEvent;
import me.nikonbite.api.lib.inventory.gui.events.GUIMiscClickEvent;
import me.nikonbite.api.lib.inventory.gui.events.GUIPickupItemEvent;
import me.nikonbite.api.lib.inventory.gui.events.GUIPlaceItemEvent;
import me.nikonbite.api.lib.inventory.gui.session.GUISession;
import me.nikonbite.api.lib.inventory.gui.session.GUIState;
import me.nikonbite.api.lib.inventory.util.StackCompatibilityUtil;
...
public class InputSlot extends AbstractGUIElement {
     ...
     */
    public InputSlot(String uniqueInputSlotID, int slot, ActionHandler actionHandler) {
        super(slot);
     ...
     * @param actionHandler     The action handler that defines how this input slot should behave when interacted with
     */
    public InputSlot(String uniqueInputSlotID, ActionHandler actionHandler) {
    ...
    public ItemStack getCurrentItem(GUIState guiState) {
        ...
        if (item instanceof ItemStack && !((ItemStack) item).getType().equals(Material.AIR)) { //Should never not be an ItemStack, but it doesn't hurt to check
    ...
    public interface ActionHandler {
         ...
         */
        boolean shouldAllowAutoInsert(Player viewer, GUISession session);
         ...
         */
        void onCurrentItemChanged(GUISession guiSession, ItemStack newItem);
    }
}
