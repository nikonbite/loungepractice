...
import me.nikonbite.api.lib.inventory.gui.elements.ActionItem;
...
import me.nikonbite.api.lib.inventory.gui.guis.GUI;
    ...
    public void populateGUI(final GUISession session, final Player viewer, final Callback<Void> callback) {
        ...
        contentsProvider.genContents(viewer, page, session, new Callback<GUIContentsProvider.GUIContentsResponse>() {
            ...
            public void call(GUIContentsProvider.GUIContentsResponse contents) {
                ...
                int vacantSlot = 0;
                topLoop:
                for (GUIElement element : new ArrayList<GUIElement>(elements)) { //Iterate over cloned list since we are modifying it during the iteration (And not doing this would cause a concurrent modification exception)
                ...
                if (elements.size() > 0) {
                    BukkitAPIStarter.getInstance().getLogger().warning(elements.size() + " GUIElements were unable to be placed into a GUI. The following stack trace should help you find what went wrong");
                ...
                contentsProvider.genTitle(viewer, page, session, new Callback<String>() {
                    ...
                    public void call(String title) {
                        if (title.length() > GUIContentsProvider.MAX_TITLE_LENGTH) { //If the title is too long
                            BukkitAPIStarter.getInstance().getLogger().warning("GUI title longer than maximum length (" + GUIContentsProvider.MAX_TITLE_LENGTH + "):" + title + "! It has been truncated!");
